...
const seriesOptions = computed(() => [
  ...
  ...seriesList.value.map((item) => ({ label: `${item.name} (${item.episode_count} tập)`, value: item.id })),
]);
...
const watermarkOptions = computed(() => [
  ...
  ...watermarkPresets.value.map((item) => ({ label: item.name, value: item.id })),
]);
const templateOptions = computed(() => [
  { label: "Thiết lập thủ công", value: "" },
  ...templates.value.filter((item) => item.status === "ready").map((item) => ({ label: item.name, value: item.id })),
]);
const variantOptions = computed(() => [1, 2, 3, 4, 5].map((value) => ({
  label: mode.value === "series" ? `${value} tập` : `${value} kịch bản`, value,
})));

async function loadOptions(): Promise<void> {
  try {
    const [series, music, watermarks, templateRows] = await Promise.all([
      api<SeriesRow[]>("/v1/series"), api<MusicTrack[]>("/v1/music"),
      api<WatermarkPreset[]>("/v1/watermark-presets"), api<TemplateRow[]>("/v1/templates"),
    ]);
...
function addLink(): void { linkInputs.value.push(""); }
...
function selectFiles(files: File[]): void {
  uploadItems.value.push(...files.map((file) => ({
    id: `${Date.now()}-${file.name}`, name: file.name, size: file.size, status: "pending" as const, file,
  })));
}
function removeFile(id: string): void { uploadItems.value = uploadItems.value.filter((item) => item.id !== id); }

async function waitForExtraction(projectId: number): Promise<void> {
  for (let index = 0; index < 45; index += 1) {
    await new Promise((resolve) => setTimeout(resolve, 2000));
    try {
      const detail = await api<{ sources: Array<{ status: string }> }>(`/v1/projects/${projectId}`);
      if (!detail.sources.some((source) => source.status === "extracting")) return;
    } catch {
...
async function submit(): Promise<void> {
  ...
  if (idea.value.trim().length < 10) {
    ...
    await nextTick();
    document.querySelector<HTMLTextAreaElement>("[data-mobile-idea] textarea")?.focus();
    return;
  }
  ...
  try {
    const { id } = await api<{ id: number }>("/v1/projects", {
      ...
      body: JSON.stringify({
        ...
        newSeriesName: mode.value === "series" ? newSeriesName.value.trim() || undefined : undefined,
      }),
    });
    ...
    if (showUserSources.value) {
      ...
      for (const raw of linkInputs.value) {
        ...
        await api(`/v1/projects/${id}/sources/link`, { method: "POST", body: JSON.stringify({ url: raw.trim() }) });
        hasSources = true;
      }
    }
    if (hasSources) {
...
<template>
  <div class="mds-mobile-app flex min-h-0 flex-1 flex-col overflow-hidden bg-[var(--mds-bg)] text-[var(--mds-text)]">
    ...
    <main class="min-h-0 flex-1 overflow-y-auto">
      ...
      <section class="mds-mobile-gutter-x border-b border-[var(--mds-border-light)] py-5">
        <h2 class="m-0 text-[16px] font-semibold leading-[22px]">Ý tưởng video</h2>
        <p class="m-0 mt-1 text-[12px] leading-4 text-[var(--mds-text-secondary)]">AI sẽ tạo kịch bản để bạn duyệt trước khi render.</p>
        <div class="mt-4" data-mobile-idea>
          <label class="mb-1 block text-[13px] font-medium">Ý tưởng <span class="text-[var(--mds-danger)]">*</span></label>
          <MTextarea v-model="idea" :rows="4" :maxlength="2000" :error="ideaError" placeholder="Ví dụ: 5 cách dùng AI tăng năng suất làm việc" />
        </div>
      </section>
      ...
      <section class="mds-mobile-gutter-x border-b border-[var(--mds-border-light)] py-5">
        ...
        <div class="mds-mobile-column-gap-4 mt-4 flex flex-col gap-4">
          <fieldset>
            <legend class="mb-1 text-[13px] font-medium">Nguồn kịch bản</legend>
            <MRadioGroup v-model="sourceMode" :options="[
          ...
          <template v-if="showUserSources">
            <MUpload :model-value="uploadItems" :accept="ACCEPT" :max-size-m-b="18" label="Tệp tham khảo" @select-files="selectFiles" @remove="removeFile" />
            <div>
              <label class="mb-1 block text-[13px] font-medium">Link tham khảo</label>
              <div v-for="(_, index) in linkInputs" :key="index" class="mds-mobile-row-gap-2 mb-2 flex gap-2">
                <MInput v-model="linkInputs[index]" class="min-w-0 flex-1" placeholder="https://..." inputmode="url" />
                <MButton v-if="linkInputs.length > 1" variant="icon" aria-label="Xóa link" @click="removeLink(index)"><template #icon><MIcon name="x" :size="20" /></template></MButton>
              </div>
              <MButton variant="ghost" @click="addLink"><MIcon name="plus" :size="16" /> Thêm link</MButton>
            </div>
          </template>
        </div>
      </section>
      ...
      <section class="mds-mobile-gutter-x border-b border-[var(--mds-border-light)] py-5">
        <h2 class="m-0 text-[16px] font-semibold leading-[22px]">Thiết lập video</h2>
        <div class="mds-mobile-column-gap-4 mt-4 flex flex-col gap-4">
          <label v-if="templates.length" class="block text-[13px] font-medium">Video Template<MSelect v-model="templateId" class="mt-1" :options="templateOptions" /></label>
          <p v-if="selectedTemplate && !selectedTemplate.workflow.approveGate" class="m-0 flex gap-2 rounded-lg bg-[var(--mds-warning-bg,#FFF7E6)] p-3 text-[12px] leading-4 text-[var(--mds-warning-text,#8A5A00)]"><MIcon name="alert-triangle" :size="20" class="shrink-0" />Mẫu này tự render sau khi sinh kịch bản.</p>
          <fieldset><legend class="mb-1 text-[13px] font-medium">Chế độ kịch bản</legend><MRadioGroup v-model="mode" :options="[{ label: 'Đa chiều', value: 'angles' }, { label: 'Serie', value: 'series' }]" direction="horizontal" /></fieldset>
          <label class="block text-[13px] font-medium">Số kịch bản<MSelect v-model="variantCount" class="mt-1" :options="variantOptions" /></label>
          <template v-if="mode === 'series'"><label class="block text-[13px] font-medium">Serie<MSelect v-model="seriesId" class="mt-1" :options="seriesOptions" /></label><label v-if="seriesId === ''" class="block text-[13px] font-medium">Tên serie mới<MInput v-model="newSeriesName" class="mt-1" placeholder="Nhập tên serie" /></label></template>
          <label class="block text-[13px] font-medium">Tông màu<MSelect v-model="presetHint" class="mt-1" :options="[{ label: 'AI tự chọn', value: '' }, { label: 'Midnight', value: 'midnight' }, { label: 'Noir', value: 'noir' }, { label: 'Paper', value: 'paper' }, { label: 'Aurora', value: 'aurora' }]" :disabled="!!selectedTemplate" /></label>
          <div><p class="mb-1 text-[13px] font-medium">Thời lượng mục tiêu</p><RangeField v-model="durationSec" :min="20" :max="120" :step="5" :format="(value) => `${value}s`" /></div>
          <label class="block text-[13px] font-medium">Nhạc nền<MSelect v-model="musicTrackId" class="mt-1" :options="musicOptions" /></label>
          <label class="block text-[13px] font-medium">Watermark<MSelect v-model="watermarkPresetId" class="mt-1" :options="watermarkOptions" /></label>
        </div>
      </section>

      <section class="mds-mobile-gutter-x py-5">
        <h2 class="m-0 text-[16px] font-semibold leading-[22px]">Giọng đọc</h2>
        <div class="mds-mobile-column-gap-4 mt-4 flex flex-col gap-4">
    ...
    <footer class="border-t border-[var(--mds-border-light)] bg-[var(--mds-bg)] py-2">
      <div class="mds-mobile-gutter-x mx-auto flex w-full max-w-[720px] justify-end"><MButton variant="primary" :loading="submitting" @click="submit">Tạo kịch bản</MButton></div>
    </footer>
  </div>
</template>
